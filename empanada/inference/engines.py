...
class QueueInferenceEngine:
    def __init__(
        ...
        **kwargs
        ...
        # set the queue len
        self.qlen = model.queue_len
        self.semantic_queue = FeatureDeque(self.qlen)
        self.instance_queue = FeatureDeque(self.qlen)
        assert hasattr(model, 'decode_precomputed'), 'Not a Queue model!'
        
    def _infer(self, image):
        # run inference with the queue
        # or build up the queue
        with torch.no_grad():
            # build the queue
            if self.model.instance_fpn is not None:
                semantic_fpn_features, instance_fpn_features = self.model.encode_fpn(image)
                self.semantic_queue.append(semantic_fpn_features)
                self.instance_queue.append(instance_fpn_features)
            else:
                semantic_fpn_features = self.model.encode_fpn(image)
                self.semantic_queue.append(semantic_fpn_features)
                
            if len(self.semantic_queue) < self.qlen:
                return None
            else:
                output = self.model.decode_precomputed(
                    self.semantic_queue.stacked, self.instance_queue.stacked
                )
                sem = output['sem']
                ctr_hmp = output['ctr_hmp']
                ...
                if sem.size(1) > 1: # multiclass segmentation
                    ...
                    sem = torch.sigmoid(sem) >= self.confidence_thr
                    sem = sem.long() # need integers
                
        return sem, ctr_hmp, offsets
        
    def __call__(self, image, **kwargs):
        # check that image has batch dim of 1
        if image.ndim == 3:
        ...
        # infer labels
        if len(self.semantic_queue) < self.qlen - 1:
            # build up features in the queue
            self._infer(image)
        else:
            # compute panoptic seg
            sem, ctr_hmp, offsets = self._infer(image)
